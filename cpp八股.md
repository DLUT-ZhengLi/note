...
+ `__attribute__((constructor))`：在GNU中可以使用attribute，类似编译标记。constructor表示在main之前运行。

**main函数执行之后**：  

+ 全局对象的析构函数会在main函数之后执行； 
+ 可以用 **`atexit`** 注册一个函数，它会在main 之后执行：`atexit(foo);`
...
- 确定结构体中每个元素大小可以通过下面这种方法:
  ...
  */
  ...
  //位域出现的原因是由于某些信息的存储表示只需要几个bit位就可以表示而不需要一个完整的字节
  //类型说明符  位域名：位域长度
  //位域必须存储在同一个类型中，不能跨类型。无名位域不能使用，只用作调整结构。大小为0的位域不能使用，下一个位域放于下一单元
...
* 对栈空间大小比较敏感（比如递归）的时候使用引用。使用引用传递不需要创建临时变量，开销要更小 todo
...
- new的实现过程是：首先调用名为**operator new**（调用malloc）的标准库函数，分配足够大的原始为类型化的内存，以保存指定类型的一个对象；接下来运行该类型的一个构造函数，用指定初始化构造对象；最后返回指向新分配并构造后的的对象的指针
- delete的实现过程：对指针指向的对象运行适当的析构函数；然后通过调用名为**operator delete**（调用free）的标准库函数释放该对象所用内存
...
- 注意对指针的操作，typedef char * p_char和#define p_char char *区别巨大。todo
...
- 相同变量可以在多处声明（外部变量extern），但只能在一处定义。todo
...
一个指针占内存的大小跟编译环境有关，而与机器的位数无关。todo
...
- 考虑类的情况
  ...
  - const成员函数：const对象不可以调用非const成员函数；非const对象都可以调用；不可以改变非mutable（用该关键字声明的变量可以在const成员函数中被修改）数据的值 todo
...
string str3 = "I am a string";//语句3 拷贝初始化，先为字符串”I am a string“创建临时对象，再把临时对象作为参数，使用拷贝构造函数构造str3 todo 移动
...
![](https://axiu-image-bed.oss-cn-shanghai.aliyuncs.com/img/202205220021689.png)
